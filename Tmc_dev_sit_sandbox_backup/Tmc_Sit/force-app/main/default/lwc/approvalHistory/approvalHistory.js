...
export default class ApprovalHistory extends LightningElement {
  ...
  deviceFormFactor=false;//varible for mobile view
  ...
  get columns() {
    ...
    console.log('coliumns is------------',tempColumns);
    return tempColumns;
  }

  get mobColumns() {
    let tempColumns = mobColumns;
    if(this.showComments){
      tempColumns.push({
        label: "Comments", fieldName: "comments" 
      });
  ...
  handleModalSubmit() {
    switch (this.currentModalState) {
      ...
      case modalStates.APPROVE.state:
        this.modalComment = getCommentPropertyFromModal(this);
        //If comments is not filled up show an error
