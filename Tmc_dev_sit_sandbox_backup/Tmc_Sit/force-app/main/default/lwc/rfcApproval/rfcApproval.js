...
import DEALER_MAP_CODE from '@salesforce/schema/RFC__c.Enquiry__r.Dealership__r.Dealer_Map_Code__c';
import TVM_Approval_Message from '@salesforce/label/c.TVM_Approval_Message';
...
export default class RfcApproval extends LightningElement {
    ...
    tvmApprovalMsg = TVM_Approval_Message;
    ...
    get isTVMApproval() {
        return (this.approvalType === 'TVM') ? true : false;
    }
    get pendingAmount() {
    ...
    @wire(getRecord, { recordId: '$recordId', fields: [ENQUIRY_ID, ENQUIRY_OWNER_ID, ENQUIRY_OWNER_PHONE, RFC_STATUS, ENQUIRY_FIRST_NAME, ENQUIRY_MIDDLE_NAME, ENQUIRY_LAST_NAME, RFC_TOTAL_AMT, RFC_AMT_RECIEVED, RFC_LOAN_AMOUNT, RFC_BUYING_PRICE, DEALER_MAP_CODE] })
    wiredRFC({ error, data }) {
        if (data) {
            ...
            this.dealerMapCode = getFieldValue(data, DEALER_MAP_CODE);
    ...
    handleApproveClick() {
        ...
        var validValue = true;
        var isPendingAmountNegative = false;
        if (this.isACMApproval) {
            if (this.pendingAmount < 0) {
                isPendingAmountNegative = true;
                validValue = false;
            } else {
                validValue = this.template.querySelector('[data-field="Total_Amount__c"]').checkValidity() ? validValue : false;
                validValue = this.template.querySelector('[data-field="Amount_Received__c"]').checkValidity() ? validValue : false;
            }
        ...
        if (validValue) {
            ...
            var errorMsg = '';
            if (isPendingAmountNegative) {
                errorMsg = 'Invalid amount on pending amount field';
            } else {
                errorMsg = 'Check your input and try again.';
            }
            this.dispatchEvent(
                new ShowToastEvent({
                    message: errorMsg,
