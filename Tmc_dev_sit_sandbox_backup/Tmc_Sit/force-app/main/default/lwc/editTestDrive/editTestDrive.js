...
export default class EditTestDrive extends NavigationMixin(LightningElement) {
    ...
    @wire(getRecord, { recordId: '$recordId', fields: ['Test_Drive__c.Enquiry__r.Dealership__r.Channel__c'] })
    wiredOpportunity({ error, data }) {
        if (data) {
            console.log('== Test Drive Data ', data);
            this.record = data;
            
            if(this.record && this.record.fields.Enquiry__r){
               let dealershipChannel = this.record.fields.Enquiry__r.value.fields.Dealership__r.value.fields.Channel__c.value;
                this.isCommercial = dealershipChannel == 'COM' ? true : false;
                        ...
                        this.updateOrCancelBooking=JSON.parse(result);
                            ...
                            if(this.updateOrCancelBooking.status==='Success')
                            {
                                ...
                                else if(this.completeType==='True')
                                {
                                    ...
                                    this.tempVar={id:this.testDriveData.Id,status:'COMPLETED',location:this.testDriveData.Location__c,selectedDate:this.todayDate,startTime:this.actualStartTime,endTime:this.actualEndTime,oldStartDate:this.testDriveData.Scheduled_Start__c,kmDriven:this.kmDriven,bookingId:this.updateOrCancelBooking.bookingId,satisfied:this.feedBackAnswerSelected,feedback:this.feedback,duration:dateTimeDiff,enquiryId:this.testDriveData.Enquiry__c,isCommercial:this.isCommercial,startMeterReading:this.startMeterReading,endMeterReading:this.endMeterReading,loadedTestDrive:this.loadedTestDrive};
                                }
                                else 
                                {
