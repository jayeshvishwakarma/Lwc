...
export default class TestDriveScheduling extends NavigationMixin(LightningElement) {
    ...
    @track endTime;
/***************************************************/
    @track variantSelected; //This variable is used to store variant of selected vehicle
    ...
    @wire(getRecord, { recordId: '$recordId', fields: ['Opportunity.Customer__r.PersonContactId','Opportunity.DSE_MSPIN__c','Opportunity.Dealership__c','Opportunity.DMS_Enquiry_Name__c','Opportunity.Customer__c','Opportunity.Line_Of_Business__c', 'Opportunity.Dealership_Channel__c'] })
    wiredOpportunity({ error, data }) {
        ...
        if (data) {
            ...
            if(this.record.fields.Dealership_Channel__c.value){
                this.dealershipChannel = this.record.fields.Dealership_Channel__c.value;
                this.isCommercial = this.dealershipChannel == 'Commercial' ? true : false;
...
findVariants()
{
    findVariant({searchVar:this.selectedVariant, channel : this.dealershipChannel, fuelType: this.selectedFuelType})
    .then(result => {
...
/*****************************************************************************************/
    ...
    handleEndMeterReading(event){
        
        this.endMeterReading = event.target.value;
        
        this.updateKmDriven();
    }

    updateKmDriven(){
        this.kmDriven = (this.endMeterReading ? parseFloat(this.endMeterReading) : 0.0 ) -  (this.startMeterReading ? parseFloat(this.startMeterReading) : 0.0)
    }

    handleLoadedTestDriveChange(event){
        this.loadedTestDrive = event.target.checked;
    }
        ...
        if(this.record.fields.DSE_MSPIN__c.value!==null && this.record.fields.DSE_MSPIN__c.value!==''&& this.record.fields.DMS_Enquiry_Name__c.value!==null && this.record.fields.DMS_Enquiry_Name__c.value!=='')
            {
                ...
                .then(result => {
                    ...
                    if(this.testDriveType==='S')
                    {
                        ...
                        {
                            this.testDriveData={contactId:this.record.fields.Customer__r.value.fields.PersonContactId.value,assetId:this.assetsId,fuelType:this.selectedFuelType ,enquiryId:this.recordId,bookingId:'',bookingDate:this.bookingDate,startTime:this.startTime,endTime:this.endTime,location:this.loc,svocCustomer:this.record.fields.Customer__c.value,status:'RESERVED',startMeterReading: this.startMeterReading, endMeterReading: this.endMeterReading, loadedTestDrive: this.loadedTestDrive, isCommercial: this.isCommercial};
                        ...
                        if(this.bookTDSlotData.status==='Success' && this.record.fields.Customer__c.value!==null && this.record.fields.Customer__c.value!=='')
                        {
                            ...
                            this.testDriveData={contactId:this.record.fields.Customer__r.value.fields.PersonContactId.value,fuelType:this.selectedFuelType ,enquiryId:this.recordId,bookingId:this.bookTDSlotData.bookingId,bookingDate:this.todayDate,startTime:this.startTime,endTime:this.endTime,location:this.loc,svocCustomer:this.record.fields.Customer__c.value,status:'COMPLETED',kmDriven:this.kmDriven,satisfied:this.feedBackAnswerSelected,feedback:this.feedback,duration:this.dateTimeDiff,startMeterReading: this.startMeterReading, endMeterReading: this.endMeterReading, loadedTestDrive: this.loadedTestDrive, isCommercial:this.isCommercial};
...
/**************************************************************************************/
    ...
    {
        /* eslint-disable no-console */
        console.log('Inside handleSearchVariant ', event.target.value);
