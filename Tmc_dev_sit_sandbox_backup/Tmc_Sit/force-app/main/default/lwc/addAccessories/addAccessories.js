...
export default class AddAccessories extends Base {
    ...
    customer({ error, data }) {
        ...
        if (data) {
         ...
         //   @track showLoyaltyRedeem = pointBalance === 'Accessories Sales' ? true:false;
    ...
    @track showLoyaltyRedeem = this.state.enquiryRecordType === 'Accessories Sales' ? true:false;

    connectedCallback() {
      //  console.log('== this.state.enquiryRecordType From Accessories ', this.state.enquiryRecordType);
        console.log('== this.state.enquiryRecordType From Accessories ', this.state);
    ...
    handleSave() {
        ...
        if(this.showLoyaltyRedeem  && this.tier && this.pointBalance && this.pointsToBeRedeemed > 0 && this.pointsToBeRedeemed > 0){
            validateLoyaltyRedemptionIsValid = this.validateInputs(".points-redeemed"); 
        ...
        // if(this.showLoyaltyRedeem  && this.pointBalance){
        //     this.state.opportunityInfo.Points_To_Be_Redeemed__c =  this.pointsToBeRedeemed;
        // }

        
        console.log(this.state.opportunityInfo);
        if (this.validateInputs(".inputCmp") & this.validateInputs(".checkValid") && validateLoyaltyRedemptionIsValid) {
            this.setData({
