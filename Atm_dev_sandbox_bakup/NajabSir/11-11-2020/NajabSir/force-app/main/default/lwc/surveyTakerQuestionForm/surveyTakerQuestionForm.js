...
export default class SurveyTakerQuestionForm extends LightningElement {
    ...
    userData({ error, data }) {
        if (data) {
            ...
            this.loggingUserName = objCurrentData.Name.value;
        }
        else if (error) {
    ...
    connectedCallback() {
        getDetails({ surveyTakerId: this.recordId, SelectedLang: this.selectedLanguage })
            .then(data => {
                if (data) {
                    console.log('== On load this.questionList ', this.questionList);
                    console.log('== User data ', data);
                    this.survey = data.survey;
                    this.surveyTaker = data.surveyTaker;
                    ...
                    for (let i = 0; i < data.responseList.length; i++) {
                        ...
                        item.Source__c = responseObj.Source__c;
                        ...
                        console.log('responseObj.Survey_Question__c ', responseObj.Survey_Question__c)
                    ...
                    if (data.quesIdWIthAnwsersMap && data.quesIdWIthQuestionObjectList) {
                        ...
                        for (var value of data.quesIdWIthQuestionObjectList) {
                            ...
                            ansList = this.quesIdAnwsersListMap.find((item) => item.key === ques.Id);
                            ...
                            if (ques.Is_Applicable_Question__c) {
                                ...
                                console.log('this.applicableCEIQuesId ', this.applicableCEIQuesId)
                            }
                    ...
                    if (!this.showAllQuesOnLoad) {
                        for (let secKey of this.secNameVsQuesList.keys()) {
                            //console.log('this.secNameVsQuesList.keys() ', this.secNameVsQuesList.keys());
    ...
    addQuestions(quesObj, srNum) {
        ...
        if (this.quesIdVsResponse.has(quesObj.Id)) {
            ...
            if (responseObj.Response__c && responseObj.Answer_Type__c === 'Multi Select') {
                ...
                let quesAddressObj = { 'sobjectType': 'Survey_Taker_CTI__c' };

                quesAddressObj.New_Address_Line_1__c = this.surveyTaker.New_Address_Line_1__c ? this.surveyTaker.New_Address_Line_1__c : '';
                quesAddressObj.New_Address_Line_2__c = this.surveyTaker.New_Address_Line_2__c ? this.surveyTaker.New_Address_Line_2__c : '';
                quesAddressObj.New_Address_Line_3__c = this.surveyTaker.New_Address_Line_3__c ? this.surveyTaker.New_Address_Line_3__c : '';
                quesAddressObj.New_Pincode__c = this.surveyTaker.New_Pincode__c ? this.surveyTaker.New_Pincode__c : '';
                quesAddressObj.New_City__c = this.surveyTaker.New_City__c ? this.surveyTaker.New_City__c : '';
                quesAddressObj.New_State__c = this.surveyTaker.New_State__c ? this.surveyTaker.New_State__c : '';
        ...
        let bottomHeight = scrollHeight + parseInt(1000);
        console.log('scrollHeight=== ', scrollHeight);
        console.log('bottomHeight=== ', bottomHeight);
        window.scrollBy(0, bottomHeight);
    ...
    addNextQues(quesId, selectedValue) {
        ...
        if (ansList) {
            for (var ans of ansList.value) {
                if (currentQuestion.Answer_Type__c === 'Text Area' || currentQuestion.Answer_Type__c === 'Date' || currentQuestion.Answer_Type__c === 'Address') {
                    ...
                    if (!nextQuestionId) {
                        endScript = ans.Survey_Answers__r ? ans.Survey_Answers__r[0].End_Script__c : ans.End_Script__c;
                    }
                    break;
        ...
        if (nextQuestionId) {
            ...
            for (let i = 0; i < nextQuestionList.length; i++) {
                ...
                let popIndex;
                let updatedDashedQuesList = [];
                let updatedNextquestionList = [];

                for (let i = 0; i < this.dashedNextquestionList.length; i++) {
                    let currentNum = this.dashedNextquestionList[i];
                    let indexdesh = currentNum.indexOf('-');
                    let leftstr = currentNum.substring(0, indexdesh);
                    let rightstr = currentNum.substring(indexdesh + 1, currentNum.length);
                    //console.log('leftstr=== ', leftstr);
                    //console.log('rightstr=== ', rightstr);
                    //console.log('currentQuestion.Sequence_Number__c=== ', currentQuestion.Sequence_Number__c);
                    if (Number(rightstr) == currentQuestion.Sequence_Number__c) {
                        popIndex = i;
                    }
                    //console.log('popIndex=== ', popIndex);
                    if (popIndex != null) {
                        // console.log('lessthan = ', Number(leftstr) < currentQuestion.Sequence_Number__c);
                        if (Number(leftstr) < currentQuestion.Sequence_Number__c) {
                            updatedDashedQuesList.push(currentNum);
                            updatedNextquestionList.push(this.newNextquestionList[i]);
                        }
                    } else {
                        updatedDashedQuesList.push(currentNum);
                        updatedNextquestionList.push(this.newNextquestionList[i]);
                    }
                    //console.log('updatedDashedQuesList=== ', updatedDashedQuesList);
                    //console.log('updatedNextquestionList=== ', updatedNextquestionList);

                }

                this.dashedNextquestionList = [];
                this.dashedNextquestionList = updatedDashedQuesList;

                this.newNextquestionList = [];
                this.newNextquestionList = updatedNextquestionList;


                this.dashedNextquestionList.push(currentQuestion.Sequence_Number__c + '-' + nextapplicableQues.Sequence_Number__c);
            ...
            console.log('this.dashedNextquestionList=== ', this.dashedNextquestionList);
        ...
        else if (currentQuestion.Is_End__c && endScript) {
            console.log('this.loggingUserName=== ', this.loggingUserName);
            let newEndScript = endScript.replace("{{!User.Name}}", this.loggingUserName);
            this.CompleteMsg = newEndScript;
    ...
    handleChange(event) {
        ...
        if (!this.quesIdVsResponse.has(quesId)) {
            ...
            responseObj.Source__c = 'SF';
            this.quesIdVsResponse.set(quesId, responseObj);
            console.log('== quesIdVsResponse ', this.quesIdVsResponse);

        } else if (this.showAllQuesOnLoad && !(currentQuestion.isStart__c && (selectedIndx != (this.isStartQuestionList.length - 1)))) {
                ...
                if (ansList) {
                    for (var ans of ansList.value) {
                        if (ans.Next_Question__c) {
                            ...
                            this.dashedNextquestionList.pop();
                        }
                    }
                }
            ...
            console.log('== this.questionList ', this.questionList);
        } else if (!this.showAllQuesOnLoad) {
            let responseObj = this.quesIdVsResponse.get(quesId);
            responseObj.Response__c = selectedValue;
            this.quesIdVsResponse.set(quesId, responseObj);
            console.log('== quesIdVsResponse@ ', this.quesIdVsResponse);
                ...
                if (nextQuestionId) {
                    ...
                    for (var nextque of nextQuestionList) {
                        ...
                        for (let i = 0; i < this.dashedNextquestionList.length; i++) {
                            ...
                            if (rightstr === currentQuestion.Sequence_Number__c) {
                                popIndex = i;
                            }
                            if (popIndex != null) {
                                if (leftstr < currentQuestion.Sequence_Number__c) {
                                    updatedDashedQuesList.push(currentNum);
                                    updatedNextquestionList.push(this.newNextquestionList[i]);
                                }
                            } else {
        ...
        for (let i = 0; i < reponseList.length; i++) {
            ...
            if (ques.Answer_Type__c === "Address") {
                ...
                if (data) {
                    ...
                    addressTakerData.New_Address_Line_1__c = parseResult.New_Address_Line_1__c ? parseResult.New_Address_Line_1__c : '';
                    addressTakerData.New_Address_Line_2__c = parseResult.New_Address_Line_2__c ? parseResult.New_Address_Line_2__c : '';
                    addressTakerData.New_Address_Line_3__c = parseResult.New_Address_Line_3__c ? parseResult.New_Address_Line_3__c : '';
                    addressTakerData.New_City__c = parseResult.New_City__c ? parseResult.New_City__c : '';
                    addressTakerData.New_State__c = parseResult.New_State__c ? parseResult.New_State__c : '';
                    addressTakerData.New_Pincode__c = parseResult.New_Pincode__c ? parseResult.New_Pincode__c : '';
                }
            }
        }
        if (this.isCEI) {
    ...
    validateAddress(event) {
        ...
        let validclick = this.template.querySelector("[data-id='content']").validate().then(result => {
            ...
            if (result.isValid) {
                ...
                if (selectedValue) {
                    let addressObj = {};
                    addressObj.New_Address_Line_1__c = selectedValue.New_Address_Line_1__c ? selectedValue.New_Address_Line_1__c : this.surveyTaker.New_Address_Line_1__c;
                    addressObj.New_Address_Line_2__c = selectedValue.New_Address_Line_2__c ? selectedValue.New_Address_Line_2__c : this.surveyTaker.New_Address_Line_2__c;
                    addressObj.New_Address_Line_3__c = selectedValue.New_Address_Line_3__c ? selectedValue.New_Address_Line_3__c : this.surveyTaker.New_Address_Line_3__c;
                    addressObj.New_Pincode__c = selectedValue.New_Pincode__c ? selectedValue.New_Pincode__c : this.surveyTaker.New_Pincode__c;
                    addressObj.New_City__c = selectedValue.New_City__c ? selectedValue.New_City__c : this.surveyTaker.New_City__c;
                    addressObj.New_State__c = selectedValue.New_State__c ? selectedValue.New_State__c : this.surveyTaker.New_State__c;
                    this.onBlurANDBtnClick(quesId, addressObj, selectedIndx, this.surveyTaker);
                }
            } else {
                this.dispatchEvent(
                    new ShowToastEvent({
    ...
    onBlurANDBtnClick(quesId, selectedValue, selectedIndx, currentQuestion) {
        ...
        if (!this.quesIdVsResponse.has(quesId)) {
            ...
            responseObj.Unique_ID__c = this.recordId + quesId;
            responseObj.Source__c = 'SF';
            responseObj.Answer_Type__c = 'Address';
            this.quesIdVsResponse.set(quesId, responseObj);
            console.log('== quesIdVsResponse ', this.quesIdVsResponse);
            if (this.showAllQuesOnLoad) {
                this.addNextQues(quesId, selectedValue);
