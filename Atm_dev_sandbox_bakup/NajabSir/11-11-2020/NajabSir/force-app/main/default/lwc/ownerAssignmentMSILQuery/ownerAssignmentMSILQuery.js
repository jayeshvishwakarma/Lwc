...
candelete=true;
selectedCloseWithoutSN=false; // This field is added to show hide sub category for Closed Without SN (Added by Sunit for Suzuki Connect)
closedWithoutSNList;
@wire(getRecord, { recordId: '$recordId', fields: [ PREVIOUS_OWNER_FIELD, MSIL_NON_SF_FIELD,CHANNEL,additionalInfo] })
...
handleClosedWithoutSNSubtypeChange(event){
...
handleSubmit(event) {
    ...
    /*else if(this.case.data.recordTypeInfo.name=='Suzuki Connect' && this.pendingCustomerQueuId && this.value==='Pending Panasonic'){
        ...
        fields.IsStopped = false;
        fields.Additional_Information__c=this.caseAdditionalInfo;
        console.log('Test add info');
        console.log(this.caseAdditionalInfo);
        console.log(JSON.stringify(fields));
    }
        ...
        fields.Id=this.recordId;
        fields.Status='Closed without SN';
        fields.Suzuki_Connect_Remark__c=this.remarks;
        fields.MSIL_Remarks__c=null;
        fields.Closed_Without_SN_Subtype__c=this.selectedClosedWithoutSNSubtype;
        fields.Additional_Information__c=this.caseAdditionalInfo;
    }
    console.log(fields);
