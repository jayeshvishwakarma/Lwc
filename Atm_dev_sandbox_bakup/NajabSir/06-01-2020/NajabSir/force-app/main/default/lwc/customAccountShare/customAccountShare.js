...
export default class CustomAccountShare extends NavigationMixin(LightningElement) {
    ...
    get isOTPRequired() { return this.status === STATUS_OTP_REQUIRED; }
    get isOTPVerified() { return this.status === STATUS_OTP_VERIFIED; }
    get isRecordShared() { return this.status === STATUS_RECORD_SHARED; }

    handleVerifyOTPSuccess(event) {
        this.status = STATUS_OTP_VERIFIED;
        this.executeAction(shareCustomerRecord, { recordId: this.recordId, otp: event.detail }).then(() => {
