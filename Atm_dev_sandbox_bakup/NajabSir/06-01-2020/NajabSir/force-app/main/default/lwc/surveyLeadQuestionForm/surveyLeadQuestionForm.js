...
export default class SurveyLeadQuestionForm extends LightningElement {
    ...
    wiredUserList({ data, error }) {
        if (data && data.survey) {
            ...
            console.log('***' + data.surveyTaker.Status.toLowerCase());
            console.log('***' + this.isFollowUp);
    ...
    addNextQues(quesId, selectedValue) {
        ...
        if (nextQuestionId) {
            ...
            console.log('this.isFormFinished', this.isFormFinished);
        }
        ...
        if (this.showAllQuesOnLoad) {
            ...
            if (targetFieldName !== undefined && targetFieldName == 'Enq_Model__c') {
                setTimeout(() => {
                    ...
                    this.selectedModelLabel = event.target.options.find(opt => opt.value === selectedValue).label
