trigger twod_AccountCreationTrigger on twod__Warranty_Registration__c (before Insert) {
    ...
    if(!Test.isRunningTest() && !Automation_Controller__c.getOrgDefaults().twod_AccountCreationTrigger_Trigger_enab__c)  return;
    
    twod_TriggerControl__c mc = twod_TriggerControl__c.getInstance( userinfo.getProfileId());
    if(mc.twod_AccountCreationTrigger__c){
        
        List<twod__Warranty_Registration__c> wrList=new List<twod__Warranty_Registration__c>();
        String BL_address1;
        String BL_address2;
        String BL_city;
        String BL_state;
        ...
        String accName;    
        Map<String,twod__Warranty_Registration__c> wrMap=new Map<String,twod__Warranty_Registration__c>();    
        for(twod__Warranty_Registration__c wr:Trigger.new)
        {        
        ...
        for(String s:accNameList){
            if(!accountPre.contains(s)){
                twod__Warranty_Registration__c wr=wrMap.get(s);
                BL_address1=wr.Address_1_BL__c;
                BL_address2=wr.Address_2_BL__c;
                BL_city=wr.City_BL__c;
                BL_state=wr.State_BL__c;
        ...
        {        
            ...
            accName=BL_address1+','+BL_city+','+BL_state+','+BL_postalcode;         
            if(wr.twod__Customer__c==null)
                wr.twod__Customer__c=accMap.get(accName);
        }        
    }
}